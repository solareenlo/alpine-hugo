...
# build image
docker build --no-cache=true --pull --tag solareenlo/hugo:latest .

# verify image build
docker images | grep solareenlo/hugo | grep latest
...
then
  ...
  # open hub.docker.com
  xdg-open https://hub.docker.com/r/solareenlo/hugo/builds/ > /dev/null
  xdg-open https://hub.docker.com/r/solareenlo/hugo/~/settings/automated-builds/ > /dev/null
fi
